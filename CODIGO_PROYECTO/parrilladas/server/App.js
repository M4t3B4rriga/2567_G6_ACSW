...
app.post("/api/empleado", (req, res) => {
  const { usuario, password, selectedCargo } = req.body;
  const query =
    "INSERT INTO EMPLEADO (USUARIO_EMP, CONTRASENA_EMP, CARGO_EMP) VALUES (?, ?, ?)";

  connection.query(
    query,
    [usuario, password, selectedCargo],
    (error, result) => {
      if (error) {
        ...
        res.json({
          USUARIO_EMP: newEmpleado.USUARIO_EMP,
          CONTRASENA_EMP: newEmpleado.CONTRASENA_EMP,
          CARGO_EMP: newEmpleado.CARGO_EMP,
        });
      }
    }
  );
...
app.put("/api/settings", (req, res) => {
  ...
  const query = "UPDATE SETTINGS SET PRECIO_EXTRA_SE = ?, NUM_MESAS_SE = ? WHERE ID_SE = 1";

  connection.query(query, [newSettings.PRECIO_EXTRA_SE, newSettings.NUM_MESAS_SE], (error, result) => {
    if (error) {
